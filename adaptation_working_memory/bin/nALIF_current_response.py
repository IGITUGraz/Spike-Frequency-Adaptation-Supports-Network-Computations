...
tf.app.flags.DEFINE_integer('seq_len', 2000, 'Number of character steps')
...
def update_plot(plot_result_values, batch=0, n_max_neuron_per_raster=20, n_max_synapses=FLAGS.n_adaptive):
    ...
    ax = ax_list[-2]
    ax.grid(color='black', alpha=0.15, linewidth=0.4)
    ax.set_ylabel('membrane potential', fontsize=fs)
    ...
    ax.plot(sub_data[:, :], color='k', label='output', alpha=0.6, linewidth=1)
    ax.axis([0, presentation_steps[-1], np.min(sub_data[:, :]),
             np.max(sub_data[:, :])])  # [xmin, xmax, ymin, ymax]
    ax.get_yaxis().set_label_coords(ylabel_x, ylabel_y)
